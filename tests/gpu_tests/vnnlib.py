...
def get_io_nodes(onnx_model):
    ...
    assert len(
        inputs) == 1, f"expected single onnx network input, got: {inputs}"
    input_name = inputs[0]

    outputs = [o.name for o in sess.get_outputs()]
    assert len(
        outputs) == 1, f"expected single onnx network output, got: {outputs}"
...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs):
    ...
    for line in lines:
        ...
        if groups:
            ...
            for rv_tuple in rv:
                update_rv_tuple(rv_tuple, op, first, second, num_inputs,
                                num_outputs)
        ...
        for rv_tuple in old_rv:
            for c in conjuncts:
                ...
                for i in range(count):
                    ...
                    update_rv_tuple(rv_tuple_copy, op, first, second,
    ...
    for rv_tuple in merged_rv.values():
        ...
        for d in range(num_inputs):
            ...
            assert r[0] != -np.inf and r[
                1] != np.inf, f"input X_{d} was unbounded: {r}"
            box.append(r)
