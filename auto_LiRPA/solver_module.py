...
def _build_solver_input(self: 'BoundedModule', node):
    ...
    if node.perturbation.norm == 2.0:
        ...
        self.solver_model.addQConstr(
            grb.quicksum((var_x - input_x)**2
                        for var_x, input_x in zip(flat_inp_gurobi_vars, flat_node_value))
            <= node.perturbation.eps**2,
            name="L2_norm_constraint"
        )
