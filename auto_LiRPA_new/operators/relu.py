...
class BoundTwoPieceLinear(BoundOptimizableActivation):
        ...
        for start_node in start_nodes:
            ...
            if sparsity <= minimum_sparsity * size_s and self.use_sparse_spec_alpha:
                ...
                if L2_variable_shape:
                    ...
                    self.lambda_out[ns] = torch.ones([self.alpha_size, sparsity + 1, batch_size],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
                ...
                if L2_variable_shape is not None:
                    ...
                    self.lambda_out[ns] = torch.ones([self.alpha_size, size_s, batch_size],
                                    dtype=torch.float, device=ref.device, requires_grad=True)
    ...
    def construct_L2_variables(self, w, g, lambda_w, lambda_in, z, neuron_radius, neuron_center, sign=+1):
        # a = torch.min(
        #     -neuron_radius*sign*w + neuron_radius*sign*g - lambda_w*z - lambda_in*neuron_center,
        #     torch.zeros_like(g)
        # )
        # b = torch.min(
        #     -neuron_radius*sign*g + lambda_w*z + lambda_in*neuron_center,
        #     torch.zeros_like(g)
        # )
        # output = torch.maximum(a**2, b**2)/(lambda_w+lambda_in)

        psi = torch.minimum(sign*(g-w) - lambda_w*z, sign*(-g) + lambda_w*z).clamp_(max=0)
        output = psi**2 / lambda_w
        return output
    ...
    def reduce_bias_based_on_L2_norm(self, w, g, L2_center, rho, lambda_out, lambda_in, start_node, output_shape, inputs, sign=+1):
        ...
        # Rescale the Linf center.
        # Linf_center = Linf_center / Linf_radius
        # L2_center = L2_center / Linf_radius
        squared_L2 = torch.sum(L2_center ** 2, dim=tuple(range(1, L2_center.ndim)))
        if isinstance(w, Patches):
            ...
            L2_center = inplace_unfold(L2_center, (patch_size[-2], patch_size[-1]), w.stride, w.padding, w.inserted_zeros, w.output_padding).unsqueeze(0).expand(patch_size)
            Linf_radius = inplace_unfold(Linf_radius, (patch_size[-2], patch_size[-1]), w.stride, w.padding, w.inserted_zeros, w.output_padding).unsqueeze(0).expand(patch_size)
            Linf_center = inplace_unfold(Linf_center, (patch_size[-2], patch_size[-1]), w.stride, w.padding, w.inserted_zeros, w.output_padding).unsqueeze(0).expand(patch_size)
            w_ = w.patches
            g_ = g.patches
            lambda_out = unfold_L2_variables(lambda_out, w)
            Linf_radius = unfold_L2_variables(Linf_radius, w)
            Linf_center = unfold_L2_variables(Linf_center, w)
            L2_center = unfold_L2_variables(L2_center, w)
            if lambda_out.shape[0] != w_.shape[0]:
                lambda_out = lambda_out.view(-1, lambda_out.shape[1])
                L2_center = L2_center.view(-1, *patch_size[4:])
                Linf_center = Linf_center.view(-1, *patch_size[4:])
                Linf_radius = Linf_radius.view(-1, *patch_size[4:])
        else:
            w_ = w
            g_ = g     
        if w_.ndim == 7:
        ...
        result = self.construct_L2_variables(w_, g_, lambda_w, lambda_in, L2_center, Linf_radius, Linf_center, sign)
        # Compute the L2 norm along all non-batch dimensions.
        if isinstance(w, Patches):
            # Calculate the L2 norm for each batch.
            result_1 = (lambda_out*(rho**2 - squared_L2)).view(output_shape)
            result_2 = torch.sum(result,  dim=tuple(range(w_.ndim-3, w_.ndim))).view(output_shape)
            # result_3 = torch.sum(lambda_in * (1 - Linf_center**2),  dim=tuple(range(w_.ndim-3, w_.ndim))).view(output_shape)
            # z = sign*1/2*(result_1 + result_2 + result_3)
        else:
            result_1 = lambda_out*(rho**2 - squared_L2)
            result_2 = torch.sum(result, dim=tuple(range(2, w_.ndim))).view(output_shape)
            # result_3 = torch.sum(lambda_in * (1 - Linf_center**2), dim=tuple(range(2, w_.ndim))).view(output_shape)
            # z = sign*1/2*(result_1 + result_2 + result_3)
    ...
    def bound_backward(self, last_lA, last_uA, x=None, start_node=None,
        ...
        if self.input_rho is not None and self.opt_stage in ['opt', 'reuse'] and self.L2_stage in ['opt']:
            ...
            if last_uA is not None:
                ...
                ubias = self.reduce_bias_based_on_L2_norm(last_uA, uA, self.xc, self.input_rho, selected_lambda_out[1], full_lambda_in, start_node, ubias.shape, x, sign=+1)
            if last_lA is not None:
                ...
                lbias = self.reduce_bias_based_on_L2_norm(last_lA, lA, self.xc, self.input_rho, selected_lambda_out[0], full_lambda_in, start_node, lbias.shape, x, sign=-1)
    ...
    def interval_propagate(self, *v):
        ...
        for input_node in self.inputs:
            if hasattr(input_node, 'output_rho') and input_node.output_rho is not None:
                ...
                self.output_rho = input_node.output_rho
                break
