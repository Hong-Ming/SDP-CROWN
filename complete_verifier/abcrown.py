...
class ABCROWN:
    ...
    def bab(self, data_lb, data_ub, c, rhs,
        ...
        if data_dict is not None:
            if isinstance(data_dict['eps'], torch.Tensor):
                assert data_dict['eps'].numel() == 1
            else:
    ...
    def complete_verifier(
        ...
        for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
            ...
            if isinstance(properties[0][0], dict):
                # A dict is possible when the speicification is not Linf norm or box,
                # but general Lp norm.
                # In this case, data_min/data_max will not be actually used.
                # What matters is the eps and norm.
                # FIXME: better data structure to represent input properties.
                def _get_item(properties, key):
                    # Concat over the batch dimension. Each item in shape (batch, ...).
                    return torch.concat([
                        item[key] for item in properties[0]], dim=0)
